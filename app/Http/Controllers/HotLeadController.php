                ...
                if($lead){
                    ...
                    if($lead_arr){
                        ...
                        if($validatedData['lead_status'] == 2){
                            ...
                            // $lead_discusin_updte = DB::table('tbl_lead_discsn')->where('lead_id',$validatedData['lead_id'])->update(['description'=> $validatedData['lead_data'],'call_duration'=> $validatedData['formattedTime'],'total_duration'=> $total_time_taken,'call_count'=> $lead_discsn_table->call_count + 1,'updated_at'=> $istTime,'lead_status_id' => $validatedData['lead_status']]);
                        ...
                        // $lead_lmt = DB::table('lead_log')->select('lead_action_dates')->where('lead_id', $validatedData['lead_id'])->where('lead_status_id', 1)->orderBy('id', 'asc')->first();
                    


                    }
                    return redirect()->route('hotLeadPage')->with('success', 'Lead is updated.');
                }
        }catch(\Exception $e){
